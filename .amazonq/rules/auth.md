...
## Key Rules
- Every tRPC procedure MUST call `requireRole()` â€” no exceptions
- `requireRole` checks `acceptedAt IS NOT NULL` â€” pending invites don't grant access
...
## Invite Flow
Admin sends invite â†’ system generates token + sends email â†’ user clicks link â†’ new user creates account / existing user gets project added â†’ JWT session issued
