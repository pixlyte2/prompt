# Coding Standards â€” Project Turtle

## Code Philosophy
Write code as if the next person to read it has never seen this project. Simplicity is the highest virtue â€” clever code is a liability, clear code is an asset.
...
- Never comment what code does â€” comment why

## Error Handling
- NEVER swallow errors silently â€” log at minimum, throw if appropriate
...
- Use `select` to fetch only needed fields â€” never bare `findMany()`
- Batch inserts: `createMany()` for bulk operations
- Add indexes for any field used in WHERE or ORDER BY

## Frontend
- Tailwind CSS classes only â€” no inline styles, no custom CSS files
...
## Testing â€” REQUIRED For Every File
- Every `.ts` file gets a `.test.ts` file alongside it
- Vitest with vi.mock() for externals â€” never hit real URLs or databases
