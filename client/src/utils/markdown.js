export function renderMarkdown(text, isDarkMode = false) {
  ...
  const rendered = text
    ...
    .replace(/\*(.+?)\*/g, '<em>$1</em>')
    .replace(/^\* (.+)$/gm, `<li style="margin-left:1.25rem;margin-bottom:0.375rem;list-style-type:disc;color:${textColor}">$1</li>`)
    .replace(/^- (.+)$/gm, `<li style="margin-left:1.25rem;margin-bottom:0.375rem;list-style-type:disc;color:${textColor}">$1</li>`)
    .replace(/^\d+\. (.+)$/gm, `<li style="margin-left:1.25rem;margin-bottom:0.375rem;list-style-type:decimal;color:${textColor}">$1</li>`)
  ...
  // Wrap the entire content in a div with the correct text color for any untagged text
  return `<div style="color:${textColor}">${rendered}</div>`;
}
