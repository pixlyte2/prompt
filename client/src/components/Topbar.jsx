...
import { Clock, Eraser, ChevronDown, LogOut, Trash2, Sun, Moon, Menu } from "lucide-react";
...
export default function Topbar({ title = "Dashboard", icon: Icon, onCacheClear, onOpenMobileNav }) {
  ...
  return (
    <>
      ...
      >
        <div className="px-3 sm:px-6 min-h-16 flex items-center justify-between gap-2 sm:gap-3 py-0">
          <div className="flex items-center gap-2 sm:gap-2.5 min-w-0 flex-1">
            {onOpenMobileNav && (
              <button
                type="button"
                onClick={onOpenMobileNav}
                className="md:hidden flex-shrink-0 p-1.5 -ml-1 rounded-lg text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
            ...
            {Icon && (
              <div className="flex w-8 h-8 rounded-lg bg-gradient-to-br from-blue-100 to-blue-200 dark:from-blue-900/50 dark:to-blue-800/50 items-center justify-center shadow-sm flex-shrink-0">
            ...
            <div className="flex flex-col min-w-0 flex-1 leading-tight">
              ...
              <p className="text-[11px] text-gray-500 dark:text-gray-400 sm:hidden truncate">
                {getFormattedDateShort()}
              </p>
            </div>
          </div>

          <div className="flex items-center justify-end gap-1 sm:gap-1.5 md:gap-2 flex-shrink-0 flex-nowrap text-sm text-gray-700 dark:text-gray-300">
            <button
              type="button"
            ...
            {!isVoiceOverRole && (
              <>
                <button
                  ...
                  className="relative p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                  title={
                    historyCount > 0
                      ? `History (${historyCount > 99 ? "99+" : historyCount})`
                      : "History"
                  }
                  aria-label={
                    historyCount > 0
                      ? `History, ${historyCount > 99 ? "99+" : historyCount} items`
                      : "History"
                  }
                >
                  <Clock size={18} />
                  {historyCount > 0 && (
                    <span className="absolute -top-0.5 -right-0.5 text-[10px] leading-none px-1 py-0.5 bg-blue-600 text-white rounded-full min-w-[1rem] text-center">
