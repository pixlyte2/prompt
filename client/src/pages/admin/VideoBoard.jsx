...
const FORMAT_PILL = {
  ...
  article: "bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-300",
};
...
function ContentModal({ open, onClose, onSaved, channelTypes, editTask }) {
  ...
  if (open && !initialized) {
    if (editTask) {
      ...
      setContentFormat(editTask.contentFormat || "");
            ...
            <div className="flex gap-1.5 flex-wrap">
              {FORMAT_OPTIONS.map((f) => (
                <button
                  key={f.value}
                  type="button"
                  onClick={() => setContentFormat(contentFormat === f.value ? "" : f.value)}
        ...
        <div className="flex-shrink-0 flex items-center gap-2 flex-wrap">
          <div className="flex items-center bg-gray-100 dark:bg-gray-800 rounded-lg p-0.5">
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${viewMode === "schedule" ? "bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm" : "text-gray-500 dark:text-gray-400"}`}
            >
              Schedule
            </button>
            <button
              onClick={() => setViewMode("completed")}
              className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all inline-flex items-center gap-1 ${viewMode === "completed" ? "bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm" : "text-gray-500 dark:text-gray-400"}`}
            >
              Completed
              {stats.completed > 0 && (
                <span className="text-[9px] bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300 px-1.5 rounded-full">{stats.completed}</span>
              )}
            </button>
          </div>
          ...
          <div className="relative flex-1 min-w-[120px] max-w-xs">
            ...
            <input
              ...
              placeholder="Search…"
              className="w-full pl-7 pr-7 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-xs text-gray-900 dark:text-white placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/40 transition-colors"
            />
            {search && (
          ...
          <div className="flex items-center gap-1.5 ml-auto">
            ...
            {filtered.length > 0 && (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600 transition-colors shadow-sm"
                title="Export as CSV"
              >
                <Download size={13} />
            ...
            <button
              onClick={fetchTasks}
              disabled={loading}
              className="p-1.5 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 disabled:opacity-50 transition-colors"
              title="Refresh"
            >
