  ...
  return (
    <AdminLayout title="Dashboard" titleInfo={`Good ${greeting}, ${user?.name || "Admin"}`} icon={BarChart3}>
      <div className="flex flex-col gap-3">
        ...
        {/* Recent AI Chat & Generations — side by side */}
            ...
            <div>
              ...
              <div className="space-y-2">
                {recentHistory.map((item) => (
                  <div key={item.id} onClick={() => navigate("/admin/ai-chat", { state: { promptId: item.promptId, sourceText: item.sourceText, videoLength: item.videoLength, aiModel: item.aiModel } })}
                    ...
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-1 min-w-0">
                        {(item.channel || item.title?.split(" - ")[0]) && (
                          <span className="text-xs font-medium px-1.5 py-0.5 rounded truncate bg-blue-50 text-blue-700">{item.channel || item.title?.split(" - ")[0]}</span>
                        )}
                        {(item.promptType || item.title?.split(" - ")[1]) && (
                          <span className="text-xs font-medium px-1.5 py-0.5 rounded truncate bg-violet-50 text-violet-700">{item.promptType || item.title?.split(" - ")[1]}</span>
                        )}
                        {(item.subType) && (
